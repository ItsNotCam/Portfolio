      ...
      <div id="projects" className="flex flex-grow overflow-auto">
        ...
        <div className="flex flex-col gap-4 text-custom-text-300 mx-4 my-8 cursor-default" id="project-list">
          {ProjectList.map((project, index) => (
						<div className="transition-opacity duration-300 flex flex-row gap-4 max-w-[800px]  rounded-lg" id={`project-item-${project.name}`}>
							...
							<div id="card" className="p-4 project-item flex flex-col gap-2 bg-custom-off-dark-300/5 backdrop-blur-lg" key={`project-${index}`}>
								<div className="flex flex-row items-center gap-1" onClick={() => updateSelection(project.name)}>
									<a target="_blank" href={project.githubLink} className="text-custom-text-300 hover:text-custom-text-100 transition-colors" style={{ display: project.githubLink ? "block" : "none" }}>
										<GitHubIcon height="2rem" width="2rem" />
									</a>
									<a target="_blank" href={project.demoLink} className="text-custom-text-300 hover:text-custom-text-100 transition-colors" style={{ display: project.demoLink ? "block" : "none" }} >
